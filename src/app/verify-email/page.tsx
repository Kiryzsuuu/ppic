"use client";

import { useEffect, useMemo, useRef, useState } from "react";
import { useRouter } from "next/navigation";

type ApiRes<T> = { ok: true; data: T } | { ok: false; error: { message: string; details?: unknown } };

export default function VerifyEmailPage() {
  const router = useRouter();
  const [code, setCode] = useState("");
  const [loading, setLoading] = useState(false);
  const [info, setInfo] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const [attempts, setAttempts] = useState(0);
  const locked = attempts >= 3;

  const inputsRef = useRef<Array<HTMLInputElement | null>>([]);

  const digits = useMemo(() => {
    const raw = code.replace(/\D/g, "").slice(0, 6);
    return Array.from({ length: 6 }, (_, i) => raw[i] ?? "");
  }, [code]);

  function setDigit(idx: number, v: string) {
    const only = v.replace(/\D/g, "");
    const arr = [...digits];
    arr[idx] = only.slice(-1);
    const merged = arr.join("").replace(/\D/g, "").slice(0, 6);
    setCode(merged);
  }

  function focusAt(i: number) {
    inputsRef.current[i]?.focus();
    inputsRef.current[i]?.select();
  }

  async function requestOtp() {
    setLoading(true);
    setError(null);
    setInfo(null);
    const res = await fetch("/api/auth/otp/request", { method: "POST" });
    const json = (await res.json().catch(() => null)) as ApiRes<{ expiresAt: string; delivery?: "sent" | "logged" }> | null;
    if (!res.ok || !json || !json.ok) {
      setError(json && !json.ok ? json.error.message : "Gagal mengirim OTP");
      setLoading(false);
      return;
    }
    if (json.data.delivery === "logged") {
      setInfo("SMTP belum dikonfigurasi. OTP ditampilkan di log server (terminal) untuk mode development.");
    } else {
      setInfo("OTP terkirim. Silakan cek email Anda.");
    }
    setLoading(false);
  }

  async function verifyOtp() {
    if (locked) return;
    setLoading(true);
    setError(null);
    setInfo(null);

    const res = await fetch("/api/auth/otp/verify", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code }),
    });

    const json = (await res.json().catch(() => null)) as ApiRes<{ verified: boolean }> | null;
    if (!res.ok || !json || !json.ok) {
      setError(json && !json.ok ? json.error.message : "Gagal verifikasi OTP");
      setAttempts((a) => a + 1);
      setCode("");
      focusAt(0);
      setLoading(false);
      return;
    }

    setInfo("Email berhasil diverifikasi. Mengarahkan...");
    setLoading(false);
    router.replace("/dashboard");
    router.refresh();
  }

  useEffect(() => {
    // Auto-send OTP when user lands here (best-effort), but avoid spamming refresh.
    try {
      const k = "ppic:lastOtpAutoSendAt";
      const last = Number(window.localStorage.getItem(k) || "0");
      const now = Date.now();
      if (!last || now - last > 60_000) {
        window.localStorage.setItem(k, String(now));
        requestOtp().catch(() => null);
      }
    } catch {
      requestOtp().catch(() => null);
    }
  }, []);

  useEffect(() => {
    if (locked) return;
    const merged = digits.join("");
    if (merged.length === 6 && !loading) {
      verifyOtp().catch(() => null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [digits.join(""), locked]);

  return (
    <div className="mx-auto max-w-lg">
      <h1 className="text-2xl font-semibold tracking-tight">Verifikasi Email (OTP)</h1>
      <p className="mt-1 text-sm text-zinc-600">Masukkan 6 digit OTP yang dikirim ke email Anda. Setelah verifikasi, akun otomatis aktif.</p>

      <div className="mt-6 grid gap-3 rounded-2xl border border-zinc-200 bg-white p-6">
        {info ? <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">{info}</div> : null}
        {error ? <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</div> : null}

        <div className="grid gap-1 text-sm">
          <div className="flex items-center justify-between">
            <div className="font-medium">Kode OTP (6 digit)</div>
            <div className="text-xs text-zinc-500">Percobaan: {attempts}/3</div>
          </div>

          <div className="mt-1 flex items-center gap-2">
            {digits.map((d, idx) => (
              <input
                key={idx}
                ref={(el) => {
                  inputsRef.current[idx] = el;
                }}
                inputMode="numeric"
                pattern="\\d*"
                maxLength={1}
                disabled={loading || locked}
                className="h-11 w-11 rounded-xl border border-zinc-200 text-center text-lg tracking-widest disabled:bg-zinc-50"
                value={d}
                onChange={(e) => {
                  setDigit(idx, e.target.value);
                  if (e.target.value && idx < 5) focusAt(idx + 1);
                }}
                onKeyDown={(e) => {
                  if (e.key === "Backspace" && !digits[idx] && idx > 0) {
                    focusAt(idx - 1);
                  }
                  if (e.key === "ArrowLeft" && idx > 0) focusAt(idx - 1);
                  if (e.key === "ArrowRight" && idx < 5) focusAt(idx + 1);
                }}
                onPaste={(e) => {
                  const txt = e.clipboardData.getData("text");
                  const only = txt.replace(/\D/g, "").slice(0, 6);
                  if (!only) return;
                  e.preventDefault();
                  setCode(only);
                  const nextIdx = Math.min(only.length, 5);
                  setTimeout(() => focusAt(nextIdx), 0);
                }}
              />
            ))}
          </div>

          {locked ? (
            <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
              Terlalu banyak percobaan. Silakan klik &quot;Kirim Ulang OTP&quot; untuk mendapatkan kode baru.
            </div>
          ) : (
            <div className="mt-1 text-xs text-zinc-500">Kode akan diverifikasi otomatis saat 6 digit terisi.</div>
          )}
        </div>

        <div className="flex flex-wrap gap-2">
          <button
            disabled={loading || locked || digits.join("").length !== 6}
            onClick={verifyOtp}
            className="inline-flex h-10 items-center justify-center rounded-lg bg-zinc-900 px-4 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-60"
          >
            Verifikasi
          </button>
          <button
            disabled={loading}
            onClick={() => {
              setAttempts(0);
              requestOtp();
            }}
            className="inline-flex h-10 items-center justify-center rounded-lg border border-zinc-200 bg-white px-4 text-sm hover:bg-zinc-50 disabled:opacity-60"
          >
            Kirim Ulang OTP
          </button>
          <a
            className="inline-flex h-10 items-center justify-center rounded-lg border border-zinc-200 bg-white px-4 text-sm hover:bg-zinc-50"
            href="/logout"
          >
            Logout
          </a>
        </div>

        <div className="text-xs text-zinc-500">
          Catatan: jika SMTP belum dikonfigurasi, OTP akan tampil di log server (mode development).
        </div>
      </div>
    </div>
  );
}
