...
const DAY_START_SHIFT_MS = WIB_OFFSET_MS - 60 * 1000; // WIB boundary at 00:01 (00:00 counts as previous day)
...
const DEFAULT_DETAIL_USER_LIMIT = 100;
          ...
          {series.map((_, i) => {
            ...
            const label = showAllDayTicks
              ? (() => {
                  ...
                  return isMonthBoundary ? monthDay : dayOnly;
                })()
              : (days[i]?.label ?? "");
            const isFirst = i === 0;
            const isLast = i === n - 1;
...
export default async function AdminDashboardReport({
  searchParams,
}: {
  ...
  const exportHref = `/api/admin/reports/export?from=${encodeURIComponent(dayKeys[0])}&to=${encodeURIComponent(
  ...
  const selectedDayKey =
    !selectedWeekKey && dayParam && isDayKey(dayParam) && dayKeys.includes(dayParam) ? dayParam : null;
  const hrefForDayKey = (k: string) => {
    const q = new URLSearchParams(drillBase);
    q.set("day", k);
    return `/admin/dashboard/report?${q.toString()}#detail`;
  };
  ...
  const selectedStart = selectedWeekKey
    ? dayStartFromKeyWIB(selectedWeekKey)
    : selectedDayKey
      ? dayStartFromKeyWIB(selectedDayKey)
      : null;

  let selectedEndExclusive = selectedStart
    ? selectedWeekKey
      ? addDaysUTC(selectedStart, 7)
      : addDaysUTC(selectedStart, 1)
    : null;
  ...
  const [usersPrint, logsPrint] = printMode
    ? await Promise.all([
  ...
  const [usersDetail, logsDetail] = selectedStart && selectedEndExclusive
    ? await Promise.all([
        prisma.user.findMany({
          where: { createdAt: { gte: selectedStart, lt: selectedEndExclusive } },
          select: { id: true, username: true, email: true, createdAt: true },
          orderBy: { createdAt: "desc" },
          take: DEFAULT_DETAIL_USER_LIMIT,
        }),
        prisma.auditLog.findMany({
          where: {
            action: { in: ["booking.submitted", "payment.submitted", "payment.validated"] },
  ...
  const printBookingIds = printMode ? uniq(logsPrint.filter((l) => l.action === "booking.submitted").map((l) => l.targetId).filter(isString)) : [];
  const printPaymentSubmittedIds = printMode ? uniq(logsPrint.filter((l) => l.action === "payment.submitted").map((l) => l.targetId).filter(isString)) : [];
  const printPaymentValidatedIds = printMode ? uniq(logsPrint.filter((l) => l.action === "payment.validated").map((l) => l.targetId).filter(isString)) : [];
  const printPaymentIds = printMode ? uniq([...printPaymentSubmittedIds, ...printPaymentValidatedIds]) : [];
  ...
  const printBookingSubmittedAtById = new Map<string, Date>();
  const printPaymentSubmittedAtById = new Map<string, Date>();
  const printPaymentValidatedAtById = new Map<string, Date>();
  if (printMode) {
    for (const l of logsPrint) {
      if (!l.targetId) continue;
      if (l.action === "booking.submitted") printBookingSubmittedAtById.set(l.targetId, l.createdAt);
      if (l.action === "payment.submitted") printPaymentSubmittedAtById.set(l.targetId, l.createdAt);
      if (l.action === "payment.validated") printPaymentValidatedAtById.set(l.targetId, l.createdAt);
    }
  }
  ...
  const [bookingsPrint, paymentsPrint] = printMode && (printBookingIds.length > 0 || printPaymentIds.length > 0)
    ? await Promise.all([
        printBookingIds.length
          ? prisma.booking.findMany({
              where: { id: { in: printBookingIds } },
              include: { user: { select: { username: true, email: true } }, simulator: true },
            })
          : Promise.resolve([] as BookingWithRefs[]),
        printPaymentIds.length
          ? prisma.payment.findMany({
              where: { id: { in: printPaymentIds } },
              include: {
                validatedBy: { select: { username: true } },
                booking: { include: { user: { select: { username: true, email: true } }, simulator: true } },
              },
  ...
  const bookingSubmittedRows = bookingIds
    .map((id) => {
      ...
      if (!b) return null;
      return { id, booking: b, at: bookingSubmittedAtById.get(id) ?? b.requestedAt };
    })
  ...
  const bookingSubmittedPrintRows = printBookingIds
    .map((id) => {
      const b = bookingPrintById.get(id);
      if (!b) return null;
      return { id, booking: b, at: printBookingSubmittedAtById.get(id) ?? b.requestedAt };
    })
    .filter(Boolean) as Array<{ id: string; booking: BookingWithRefs; at: Date }>;

  const paymentSubmittedPrintRows = printPaymentSubmittedIds
    .map((id) => {
      const p = paymentPrintById.get(id);
      if (!p) return null;
      return { id, payment: p, at: printPaymentSubmittedAtById.get(id) ?? p.paidAt ?? p.createdAt };
    })
    .filter(Boolean) as Array<{ id: string; payment: PaymentWithRefs; at: Date }>;

  const paymentValidatedPrintRows = printPaymentValidatedIds
    .map((id) => {
      const p = paymentPrintById.get(id);
      if (!p) return null;
      return { id, payment: p, at: printPaymentValidatedAtById.get(id) ?? p.validatedAt ?? p.createdAt };
    })
    .filter(Boolean) as Array<{ id: string; payment: PaymentWithRefs; at: Date }>;
  ...
  const chartUsersSeries = useWeeklyChart && weekBuckets ? weekBuckets.map((w) => sumSeriesForKeys(usersSeries, w.dayKeys)) : usersSeries;
  ...
  const maxUsers = Math.max(1, ...chartUsersSeries);
  ...
  const totalUsers = sum(usersSeries);
  ...
  const peakUsers = peakDay(usersSeries, dayKeys);
  ...
  const avgUsers = daysCount > 0 ? Math.round(totalUsers / daysCount) : 0;
    ...
    <div className="grid w-full min-w-0 max-w-full gap-6" data-report-page="true">
        ...
        <div className="border-b border-zinc-200 pb-3">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="text-base font-semibold">Laporan Aktivitas (Admin)</div>
      ...
      <div className="flex flex-wrap items-end justify-between gap-3" data-print-hidden="true">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-sm text-zinc-600">Admin • Range: {dayKeys[0]} – {dayKeys[dayKeys.length - 1]} ({dayKeys.length} hari) • WIB</div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <a className={ACTION_BUTTON_CLASS} href="/admin/dashboard">
            Kembali
          </a>
      ...
      <section className="min-w-0 max-w-full rounded-2xl border border-zinc-200 bg-white p-6" data-print-hidden="true">
        ...
        <form className="mt-3" method="GET" action="/admin/dashboard/report">
      ...
      <section className="min-w-0 max-w-full rounded-2xl border border-zinc-200 bg-white p-6 print-avoid-break">
        ...
        <div className="mt-3 overflow-x-auto rounded-xl border border-zinc-200" data-print-overflow="visible">
          <table className="w-full min-w-[760px] text-sm" data-print-table="fixed" data-print-wrap="true">
            ...
            <tbody>
              <tr className="border-t border-zinc-200">
                <td className="px-3 py-2 font-medium">Registrasi User</td>
                <td className="px-3 py-2 text-zinc-700">{totalUsers.toLocaleString("id-ID")}</td>
                <td className="px-3 py-2 text-zinc-700">{avgUsers.toLocaleString("id-ID")}</td>
                <td className="px-3 py-2 text-zinc-700 whitespace-normal break-words">
                  {peakUsers.value.toLocaleString("id-ID")} ({peakUsers.key})
                </td>
              </tr>
              <tr className="border-t border-zinc-200">
      ...
      {printMode ? (
        <section className="print-only min-w-0 max-w-full rounded-2xl border border-zinc-200 bg-white p-6">
          ...
          <div className="mt-4 grid gap-6">
                ...
                <table className="w-full text-sm" data-print-table="fixed" data-print-wrap="true">
                  <thead className="bg-zinc-50 text-xs text-zinc-600">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left">Registrasi</th>
                  ...
                  <tbody>
                    {dayKeys.map((k, i) => (
                      <tr key={k} className="border-t border-zinc-200">
                        ...
                        <td className="px-3 py-2 text-zinc-700">{(usersSeries[i] ?? 0).toLocaleString("id-ID")}</td>
            ...
            <div className="grid gap-2">
              <div className="text-sm font-semibold">Registrasi User (1 bulan) (maks {PRINT_TAKE.toLocaleString("id-ID")})</div>
              <div className="rounded-xl border border-zinc-200" data-print-overflow="visible">
                <table className="w-full text-sm" data-print-table="fixed" data-print-wrap="true">
                  <thead className="bg-zinc-50 text-xs text-zinc-600">
                    <tr>
                    ...
                    {usersPrint.length === 0 ? (
                      ...
                      usersPrint.map((u) => (
                        <tr key={u.id} className="border-t border-zinc-200">
                          <td className="px-3 py-2 font-medium">{dayKeyWIB(u.createdAt)}</td>
                          <td className="px-3 py-2 text-zinc-600">
                            {u.createdAt.toLocaleTimeString("id-ID", { timeZone: "Asia/Jakarta", hour: "2-digit", minute: "2-digit", hour12: false })}
                          </td>
                          <td className="px-3 py-2 text-zinc-700">{u.username}</td>
      ...
      <section id="detail" className="min-w-0 max-w-full rounded-2xl border border-zinc-200 bg-white p-6" data-print-hidden="true">
        <div className="flex flex-wrap items-end justify-between gap-2">
          ...
          {selectedChartKey ? (
            <a
              ...
              href={`/admin/dashboard/report?from=${encodeURIComponent(dayKeys[0])}&to=${encodeURIComponent(dayKeys[dayKeys.length - 1])}`}
            >
              Tutup Detail
            </a>
          ) : null}
        ...
        {selectedStart && selectedEndExclusive ? (
          <div className="mt-5 grid gap-6">
                    ...
                    <tbody>
                      {selectedWeekBreakdown.map((r) => (
                        <tr key={r.key} className="border-t border-zinc-200">
                          ...
                          <td className="px-3 py-2 text-zinc-700">{r.users.toLocaleString("id-ID")}</td>
            ...
            <div className="grid gap-2">
              <div className="text-sm font-semibold">Registrasi User (maks {DEFAULT_DETAIL_USER_LIMIT})</div>
              <div className="overflow-x-auto rounded-xl border border-zinc-200">
